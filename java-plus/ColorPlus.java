...
/**
 * The <code>ColorPlus</code> class is meant to build upon the {@link Color} class
 * by adding various utility methods and new colors to improve the creation and 
 * usage of colors in Java.
 * 
 * @author 	jonatanfontanez
 * @see 	Color
 *
 */
public abstract class ColorPlus {
	...
	/**
	 * The color orange; more orange than <code>Color.ORANGE</code>. In the sRGB space.
	 */
	 ...
	 * linear interpolation to create a proper blending ratio. Great for representing
	 * both colors equally, but not great for intermediate colors.
	 * 
	 * @param c1 first color to blend
	 * @param c2 second color to blend
	 * @param blend factor to blend colors
	 * @return a blended color <code>blend</code> of the way, in linear terms, between
	 ...
	 * the shape of an arc to create a proper blending ratio. Great for showing intermediate
	 * colors, but not great for showing base colors.
	 * 
	 * @param c1 first color to blend
	 * @param c2 second color to blend
	 * @param blend factor to blend colors
	 * @return a blended color <code>blend</code> of the way, in circular terms, between
	 * the first and second colors
	 */
	...
	/**
	 * Gives a slightly dimmer version of the input color. Similar to {@link Color#darker()},
	 * but through HSB space instead of sRGB space. Repeated dims will bring a color
	 * closer to black. Black will not be affected.
	 * 
	...
	/**
	 * Returns a color created through CMYK space. This color space is used in printing;
	 * the C refers to the value of cyan, the M to the value of magenta, the Y to the
	 * value of yellow, and the K to the value of black. All values input must be
	 * normalized.
	 * 
	...
	/**
	 * Returns a grayed-out version of the input color based on the average of
	 * the highest and lowest RGB values. This method is considered the least
	 * accurate due to loss of contrast.
	 * 
	 * @param c input color
	 * @return grayed-out color through minimum-maximum RGB average
	 */
	public static Color lightnessGrayscale(Color c) {
		int value = (int)((MathPlus.max(c.getRed(), c.getGreen(), c.getBlue()) + MathPlus.min(c.getRed(), c.getGreen(), c.getBlue()))/2);
	...
	public static Color luminosityGrayscale(Color c) {
		return new Color((int)(c.getRed()*0.2126), (int)(c.getGreen()*0.7152), (int)(c.getBlue()*0.0722));
	}
